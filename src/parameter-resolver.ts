...
import type { CliLanguageOptions } from "./cli-language-parser.ts";
import type { ConfigInputs } from "./config-loader.ts";
import { SagmalError } from "./errors.ts";

/**
 * Fully resolved translation parameters ready for DeepL API (no further resolution needed).
 */
...
function resolveTargetLanguage(
	...
	target ??= "en-US"; // Default if all else is null
...
export function resolveParameters(
	...
	const sourceLanguage = resolveSourceLanguage(cliLanguages, configInputs);
	const targetLanguage = resolveTargetLanguage(cliLanguages, configInputs);
